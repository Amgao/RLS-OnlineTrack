...
def train_owm(model, criterion, optimizer, pos_feats, neg_feats, maxiter, P4, P5, P6, W4, W5, W6, flag_old, in_layer='fc4'):
    ...
    for iter in range(maxiter):
        ...
        def pro_weight(p, x, w, w_bk, alpha=1.0, stride=1):
            r = x
...
def run_mdnet(img_list, init_bbox, gt=None, seq='seq_name ex)Basketball', savefig_dir='', display=False):
    ...
    init_optimizer = set_optimizer(model, opts['lr_init'], {'fc4':1, 'fc5':1, 'fc6':10})
    update_optimizer = set_optimizer(model, opts['lr_update'])
    update_optimizer_owm = set_optimizer(model, opts['lr_update'], {'fc4':1, 'fc5':1, 'fc6':10})
    ...
    P4 = torch.autograd.Variable(torch.eye(512 * 3 * 3 + 1).type(dtype), volatile=True)*2000.0
    P5 = torch.autograd.Variable(torch.eye(512 + 1).type(dtype), volatile=True)*2000.0
    P6 = torch.autograd.Variable(torch.eye(512 + 1).type(dtype), volatile=True)*2000.0
