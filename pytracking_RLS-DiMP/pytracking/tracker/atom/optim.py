...
class FactorizedConvProblem(optimization.L2Problem):
    ...
    def __call__(self, x: TensorList):
        ...
        residuals_inputmean = operation.conv2d(compressed_samples, filter, mode='same')
        residuals = TensorList([residuals_inputmean[0][0]]).apply(self.response_activation)
        weightsum = self.sample_weights.sum()
        inputmean = torch.sum(self.sample_weights[0].view(-1, 1, 1, 1) * residuals_inputmean[0][1], 0, True)
...
class ConvProblem(optimization.L2Problem):
    ...
    def __call__(self, x: TensorList):
        ...
        residuals_inputmean = operation.conv2d(self.training_samples, x, mode='same')
        residuals = TensorList([residuals_inputmean[0][0]]).apply(self.response_activation)
        b = torch.nonzero(self.sample_weights[0].data).size(0)
        # newweight = math.sqrt(b) * self.sample_weights.sqrt()
        newweight = self.sample_weights.sqrt()
        # weightsum = newweight.sum()
        newweight = newweight / b
        inputmean = torch.sum(newweight[0].view(-1, 1, 1, 1) * residuals_inputmean[0][1], 0, True)
        residuals = residuals - self.y
